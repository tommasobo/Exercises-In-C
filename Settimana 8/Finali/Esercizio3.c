/*
    ...
    * Testo esercizio: Realizzare un programma per il calcolo del fattoriale in modo da visualizzare le sue variabili locali
      ...
      L'obiettivo Ë quello di disegnare e implementare un formato dell'output che aiuti l'utente a capire la ricorsione.
...
unsigned long fattoriale(int n); // Prototipo della funzione che utilizzer√≤

// Questa funzione calcola il fattoriale di un numero n (parametro) e restituisce il risultato
unsigned long fattoriale(int n) // Uso un unsigned long int in quanto il fattoriale pu√≤ assumere valori molto alti
 {
    int i;
    unsigned long fatt;

    if (n == 0) // Se n Ë uguale a 0 stampo restituisco direttamente 1 che Ë il suo fattoriale
        return 1;
    else {
        for (i = 0; i < livello; i++) // Il ciclo for tiene conto del livello di indentazione
            printf("   ");
        if (n > 1) // Se n Ë maggiore di uno stampo una certa stringa
