...
int main()
{
    int n, nPrimi, divisore, i, cont, radice;
    ...
    while (nPrimi < n) {
        ...
        */
        ...
        /*
            ...
            è primo andando a verificare il resto della divisione i e tutti i numeri compresi tra due e i. Per eseguire questa
            operazione si usa una variabile divisore
        */
        else {
            /*
                Per verificare se è un numero è primo non serve farlo dividendolo per tutti i numeri fino a se stesso, ma basta farlo
                fino ad arrivare che la radice del numero è uguale al valore del divisore
            */
            radice = (int)sqrt(i) + 1; // Trovo la radice di i e la converto in un int. Aggiungo uno in caso in cui il risultato sia dispari
            for (divisore=1; radice >= divisore; divisore += 2) { // Incremento di due in quanto ho già verificato prima che non è pari
                if (i % divisore == 0)
