...
int main()
{
    ...
    // Non serve che libero la memoria allocata con il puntatore str in quanto viene già liberata precedentemente dato che gli indirizzi
...
void menu()
{
    ...
    case 5 :
        ...
        break;
    case 6 :
        for (i = 0; i < myIndex; i++) { // Libero prima lo spazio in memoria più interno
            free(student[i].name);
            free(student[i].surname);
            free(student[i].address);
            free(student[i].exams);
...
void searchSubject() // Questa funzione stampa tutti gli studenti che non hanno sostenuto un esame
{
    ...
    if (trovato == FALSE && myIndex > 0) // Se non è stato trovato neussuno studente significa che tutti hanno già svolto quell'esame
