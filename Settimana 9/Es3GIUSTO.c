...
void add() // Questa funzione permette di aggiungere uno studente al registro
{
    ...
    length = strlen(student[i].name) - 1; // Tolgo l'ultimo carattere che viene salvato dalla fgets ovvero l'andata a capo sostituendolo con il terminatore di stringa
    if (student[i].name[length] == '\n') student[i].name[length] = '\0';

    printf("Inserisci il cognome: ");
    fgets(student[i].surname, MAX, stdin);
    length = strlen(student[i].surname) - 1;
    if (student[i].surname[length] == '\n') student[i].surname[length] = '\0';

    printf("Inserisci l'indirizzo: ");
    fgets(student[i].address, MAX, stdin);
    length = strlen(student[i].address) - 1;
    if (student[i].address[length] == '\n') student[i].address[length] = '\0';
